...
opt_data = {
    ...
    "regex_mode": {
        "type": bool,
        "incompatible_enforce": {
            "enforced": [("case_sensitive", True)],
...
def editor_window(entry_name, main, config):
    ...
    ttk.Label(top, text="Enter all values, separated by newlines.").pack(
    ...
    txt = Text(
        ...
        highlightbackground=top.cget("background"),
        highlightcolor=top.cget("background"),
...
def create_survey_config(main: Tk, txt: Text = None):
    ...
    for name, old_val in old.items():
        ...
        if fdat["type"] in [str, int] or type(fdat["type"]) is list:
            ...
            entries[name].set(str(old_val))
            ttk.Radiobutton(
                top, text="True", variable=entries[name], value="True", width=6
            ).grid(row=row, column=1)
            ttk.Radiobutton(
                top, text="False", variable=entries[name], value="False", width=6
            ...
            ttk.Button(
                top,
                text="Open Editor",
                command=lambda n=name, m=main: editor_window(n, m, config),
...
def submit_survey(top: Toplevel, txt: Text = None):
    ...
    for name, var in entries.items():
        ...
        if "namemethod" in fdat.keys():
            ...
            lb_name = name.title()
        if "incompatible_enforce" in fdat.keys() and fdat["incompatible_enforce"][
            "check"
        ](val):
        ...
        try:
            ...
            showerror(
                "ValueError",
                "Invalid value {} in {} field, see README for proper values".format(
                    val, lb_name
                ),
            )
            return
